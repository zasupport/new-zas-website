...
# §753 §522 ADVISOR FINDINGS → BLOG SINK, NOT CHAT (HARD — 08/07/2026): the §522 gap battery (ctr-gap/doorway/weak-pricing/pool) auto-routes to docs/seo/advisor-gap-inbox.md (blog insights root, §751 principle for FINDINGS) + drains to §289 19:00 outstanding email (za-outstanding-reminder.sh --add; PASSIVE surfacing, NEVER the §284 auto-executor = §190). Chat gets ONE quiet line; loud battery suppressed. Self-heal still runs; §641 preflight stays (separate HARD). Enforce: za-web-blog-advisor.py --route (hook calls it) + --route-test (isolated sandbox, silence+drain+churn-guard neg-controls). Full: ~/.claude/rules/753-advisor-findings-route-to-blog-sink.md | reversible §552 --revert 753

# §795 SITE-IMPROVEMENT LOOP — PERIOD-VS-PERIOD + ADVISORY NEXT-PAGE (HARD — 21/07/2026): GSC page export USED TO OVERWRITE itself → period comparison was IMPOSSIBLE (zero dated snapshots; each day lost permanently). FIX: za-gsc-page-export.py --export now appends 1 row/page/day to ~/.za-gsc-pages-timeseries.jsonl (idempotent; same-day re-run REPLACES, never duplicates). CONSUMER: za-site-improve.py --status|--compare|--recommend|--test (ranks high-impressions + pos<=20 + CTR<2% = earned rankings leaking clicks). WIRED via feed_improve() in za-google-intel-daily.sh (LA com.zasupport.googleIntelDaily daily) → also runs za-keyword-revenue-attribution.py which was ALIVE but UNWIRED; output appends to the §753 sink docs/seo/advisor-gap-inbox.md. ⚠ ADVISORY ONLY — NEVER auto-publish (§529 doorway prune + §723 price drought both came from autonomous generation). HONEST LIMIT: with 1 day captured --compare returns UNKNOWN, which is NOT "no change". NO CRM/CMS NEEDED — the loop runs on analytics regardless of where posts live. Teaching: ~/.claude/knowledge/site-improvement-loop.md | 4 controls + registered in 07:00 regression (reachable, before exit)
