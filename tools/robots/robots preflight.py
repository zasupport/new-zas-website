...
ROBOTS_SOURCES = [
    ...
    "app/robots.txt", "public/robots.txt",
]
...
def add(section, name, status, detail="", fix=""):
    results.append({"section": section, "check": name, "status": status,
...
def check_location():
    ...
    if rc != 0:
        add(s, "Inside a git repository", FAIL,
            f"cwd is {Path.cwd()}",
    ...
    if pkg.exists():
        try:
            ...
            add(s, "Repository is a Next.js project", PASS if looks_right else FAIL,
                f"name={d.get('name','?')} next={deps.get('next','absent')}",
                "" if looks_right else "This does not look like the site repo. "
                                       "Check you are in the zasupport.com repository.")
        except Exception as exc:
            add(s, "package.json readable", FAIL, str(exc), "Repair package.json")
    else:
        add(s, "package.json present", FAIL, "not found",
    ...
    add(s, "Working tree state", WARN if n_dirty else PASS,
...
def check_toolchain(tools):
    ...
    for exe, needed in (("git", True), ("node", False), ("npm", False)):
        p = shutil.which(exe)
        add(s, exe, PASS if p else (FAIL if needed else WARN),
            p or "not found",
    ...
    if venv_py and venv_py.exists():
        ...
        add(s, "Virtual environment", FAIL,
    ...
    for mod, needed, why in (
        ...
        add(s, f"module {mod}", PASS if rc == 0 else (FAIL if needed else WARN),
...
def check_nextjs(repo):
    ...
    layout = ("App Router" if app_router else "") + \
             (" and Pages Router" if app_router and pages_router else
              "Pages Router" if pages_router else "")
    add(s, "Router layout", PASS if layout else WARN, layout or "neither app/ nor pages/ found",
    ...
    if len(found) == 1:
        ...
        add(s, "robots source", FAIL, ", ".join(found),
        ...
        add(s, "robots source", FAIL, "none of " + ", ".join(ROBOTS_SOURCES),
            "The site serves robots.txt from somewhere this tool cannot patch. "
            "Find it before running the patch stage.")

    sitemap_src = [r for r in ("app/sitemap.ts", "src/app/sitemap.ts", "app/sitemap.xml",
                               "public/sitemap.xml", "next-sitemap.config.js")
                   if (repo / r).exists()]
    add(s, "sitemap source", PASS if sitemap_src else WARN,
        ", ".join(sitemap_src) or "not found in the usual places",
        "" if sitemap_src else "Confirm how sitemap.xml is generated, the engine reads the live one")
...
def check_tools(tools):
    ...
    if tools is None:
        ...
        return
    if not tools.is_dir():
        add(s, "tools/robots present", FAIL, str(tools),
            "Run Block 1 of the runbook from inside the repo")
        return
    ...
    for f in ("robots index engine.py", "robots guard.py", "robots pressure test.py"):
        p = tools / f
        add(s, f, PASS if p.exists() else FAIL,
    ...
    for d in ("state", "inbox", "_archive"):
        p = tools / d
        add(s, f"{d}/ directory", PASS if p.is_dir() else WARN,
...
def check_config(tools, repo):
    ...
    for k in ("site", "gsc_property", "repo_path", "sitemaps", "notify_email"):
        add(s, f"key {k}", PASS if c.get(k) else FAIL,
            str(c.get(k, ""))[:70], "" if c.get(k) else "Add it in Block 2")

    prop = c.get("gsc_property", "")
    ok_shape = prop.startswith("sc-domain:") or prop.startswith("http")
    add(s, "gsc_property shape", PASS if ok_shape else FAIL, prop,
        "" if ok_shape else
        "Must be exactly as Search Console shows it. Domain property is "
    ...
    if repo:
        ...
        add(s, "repo_path matches this repo", PASS if same else FAIL,
...
def check_credentials(cfg):
    ...
    if key:
        ...
        add(s, "ANTHROPIC_API_KEY", WARN, "not in this shell but present in ~/.za-support.env",
            "Run: source ~/.za-support.env")
    else:
        add(s, "ANTHROPIC_API_KEY", FAIL, "not set and not in ~/.za-support.env",
    ...
    p = Path(sa)
    if not p.exists():
        add(s, "GSC service account file", FAIL, sa,
    ...
    try:
        ...
        add(s, "GSC service account file", PASS if email else FAIL,
            email or "no client_email in the key file",
            "" if email else "This does not look like a service account key")
        if email:
            add(s, "Service account added to the property", WARN,
        ...
        add(s, "Key file permissions", PASS if mode in ("600", "400") else WARN, mode,
...
def check_inputs(tools, cfg):
    ...
    if zips:
        add(s, "Unextracted export archive", FAIL,
    ...
    if not files:
        add(s, "Search Console export present", FAIL, f"{inbox} is empty",
    ...
    add(s, "Search Console export present", PASS,
    ...
    add(s, "URLs for this site in the export", PASS if urls else FAIL,
        f"{len(urls)} found",
        "" if urls else f"The export contains no {site} URLs. Check you exported the "
                        f"right property and the right issue.")
    if foreign:
        add(s, "URLs for other domains in the export", WARN,
    ...
    try:
        ...
        if urls:
            add(s, "Export matches this issue", PASS if blocked else WARN,
                f"{len(blocked)} of {len(urls)} export URLs are blocked by the live robots.txt",
                "" if blocked else
                "None of the exported URLs are currently blocked. Either the fix is "
...
def check_proof(tools):
    ...
    if not p.exists():
        add(s, "Pressure test has been run", FAIL, "state/pressure test.json missing",
    ...
    try:
        ...
        add(s, "Pressure test result", PASS if (neg_ok and pos_ok) else FAIL,
            f"positive {d.get('positive_passed')}/{d.get('positive_total')}, "
            f"negative {d.get('negative_passed')}/{d.get('negative_total')}",
            "" if (neg_ok and pos_ok) else
            "Do not trust the guard until every control passes")
    except Exception as exc:
    ...
    if prot.exists():
        ...
        add(s, "Protected URL set", WARN, "not generated yet",
...
def check_wiring(tools, repo):
    ...
    home = Path.home()
    points = [
        ("Skill, every session", home / ".claude" / "skills" / "robots-index-guard" / "SKILL.md", None),
        ("Skill, project scope", repo / ".claude" / "skills" / "robots-index-guard" / "SKILL.md", None),
    ...
    for name, path, needle in points:
        ...
        add(s, name, PASS if ok else FAIL, str(path),
    ...
    if sys.platform != "darwin":
        ...
        add(s, "Weekly monitor", PASS if loaded else FAIL,
            str(plist), "" if loaded else "Rerun the monitor block")
...
def check_live(cfg):
    ...
    try:
        ...
        return
    for label, url in (("robots.txt", f"{site}/robots.txt"),
        ...
        try:
            ...
            add(s, label, PASS if ok else FAIL,
            ...
            if label == "robots.txt" and ok and r.text.startswith("\ufeff"):
                add(s, "robots.txt encoding", FAIL,
...
def check_robots_structure(cfg):
    ...
    if google_named and has_wildcard:
        add(s, "Googlebot group vs wildcard", FAIL,
        ...
        add(s, "User-agent group count", WARN,
        ...
        add(s, "User-agent group structure", PASS,
    ...
    if re.search(r"(?im)^\s*Host:", txt):
        add(s, "Host directive", WARN, "present",
    ...
    for bot in ("OAI-SearchBot", "PerplexityBot", "ChatGPT-User", "Claude-SearchBot"):
        m = re.search(rf"(?ims)^\s*User-agent:\s*{re.escape(bot)}\s*$(.*?)(?=^\s*User-agent:|\Z)",
                      txt)
        if m and re.search(r"(?im)^\s*Disallow:\s*/\s*$", m.group(1)):
            add(s, f"AI search crawler {bot}", WARN, "blocked with Disallow: /",
...
def report(as_json, strict):
    ...
    if as_json:
        print(json.dumps({
            ...
            "counts": counts,
            "verdict": "NOT READY" if blocking else ("READY WITH WARNINGS" if warnings else "READY"),
            "results": results,
        }, indent=2))
    else:
        section = None
        ...
        print("\n" + "=" * 74)
        print(f"  PREFLIGHT   pass {counts[PASS]}   warn {counts[WARN]}   "
