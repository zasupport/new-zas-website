...
BANNED = {
    "visible JSON-LD": re.compile(
        r'(&quot;|")@type(&quot;|")\s*:\s*(&quot;|")(FAQPage|Question|Answer|HowTo|Article)(&quot;|")',
        re.I,
    ),
    "ld+json script text": re.compile(r"application/ld\+json", re.I),
    "visible @context": re.compile(
        r'(&quot;|")@context(&quot;|")\s*:\s*(&quot;|")https://schema\.org', re.I
    ),
    "raw code fence": re.compile(
        r"```(markdown|json|bash|html|css|javascript|typescript|jsx|tsx)\b", re.I
    ),
    ...
    "VERIFICATION block": re.compile(
        r"VERIFICATION\s*(&amp;|&|AND)\s*(LEARNING|REPORT|OUTPUT)", re.I
    ),
}
...
def run_test() -> int:
    leak_html = (
        "<html><body><p>&quot;@type&quot;: &quot;FAQPage&quot;</p><p>```json</p></body></html>"
    )
    clean_html = (
        "<html><body><p>Real article text about MacBook repair in Sandton.</p>"
...
def main():
    ...
    if not os.path.isdir(os.path.join(built_dir, "blog")):
        print(
            f"WARN: no built blog HTML at {built_dir}/blog — run `npm run build` first. Skipping (non-fatal)."
        )
        return 0
    total, leaks = scan_dir(built_dir)
    if leaks:
        print(
            f"❌ FAIL §rendered-leak-gate: {len(leaks)} of {total} built pages render BANNED content as visible text:"
        )
