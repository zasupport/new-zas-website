...
BANNED_LINE = [
    ...
    re.compile(
        r"^\s*\*{0,2}\s*(LEARNED|BETTER|WHY(\s+SUCCESS)?|REPLICATE|VERIFIED OUTPUT|KEY INSIGHT|OBSERVATION|TASK)\s*\*{0,2}\s*:",
        re.I,
    ),
    ...
    re.compile(
        r"^\s*\*{2}\s*(Internal links?|External links?|Geographic scope|Banned phrases?|UK English|FAQs?|Schema|CTAs?|Reading time|Target audience|Primary keyword|Secondary keyword|Meta description|SEO score)\*{0,2}\s*:",
        re.I,
    ),
]
LINK_LABEL = re.compile(
    r"^\s*\*{0,2}\s*(Internal|External) links?\s*(used|placed)?\s*:?\s*\*{0,2}\s*$", re.I
)
...
def sanitise(content: str, escaped: bool = False) -> str:
    ...
    for l in lines:
        if not in_s and re.search(r"<script\b", l, re.I):
            in_s = not re.search(r"</script>", l, re.I)
            continue
        if in_s:
            if re.search(r"</script>", l, re.I):
                in_s = False
            continue
        tmp.append(l)
    lines = tmp
    ...
    drop_wrap = (
        fne is not None
        and FENCE.match(lines[fne])
        and FENCE.match(lines[fne]).group(1).lower() == "markdown"
    )
...
def _iter_posts(text):
    ...
    spans = {
        m.group(1): (m.start(), ms[i + 1].start() if i + 1 < len(ms) else len(text))
...
LEAK_VIS = re.compile(
    r'"@type"|"acceptedAnswer"|application/ld\+json|<script|(JSON-?LD\s*)?FAQ(Page)?\s*(JSON-?LD\s*)?Schema|Word count\s*[:\(]|Internal Links Placed|VERIFICATION\s*(&|AND)\s*(LEARNING|REPORT|OUTPUT)',
    re.I,
)
...
def cmd_test():
    ...
    trailer = (
        "Last paragraph of real body copy ends here.\n\n---\n\n---\n\n"
    ...
    print(
        f"trailer control  — trailer removed: {'CLEAN' if not trailer_bad else 'LEFTOVER:' + str(trailer_bad)}; body kept: {body_kept}; no orphan rule: {no_orphan_rule}"
    )
...
if __name__ == "__main__":
    ...
    if "--apply" in sys.argv:
        sys.exit(cmd_apply(write=True))
    if "--dry-run" in sys.argv:
        sys.exit(cmd_apply(write=False))
    print(__doc__)
    sys.exit(2)
