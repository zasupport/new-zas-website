...
LEADTIME_RE = re.compile(
    r"\b\d{1,3}\s*-\s*\d{1,3}\s*(?:hours?|hrs?|min|mins|minutes|days?|business days?)\b"
    r"|[Tt]urnaround"
    r"|[Ll]ead[\s-]?time"
)
    ...
    for f in glob.glob(os.path.join(APP, "**", "page.tsx"), recursive=True):
        ...
        if CONTRACT_RE.search(f):
            ...
            if "<PricingNote" in src and not has_optout(src):
                violations.append(
                    f"{rel}: CONTRACT page must use <PricingNote repair={{false}} /> (device-repair clause is wrong here)"
                )
            continue
        ...
        if has_signal and not has_clause_render(src):
            violations.append(
                f"{rel}: shows price/turnaround but no machine-dependent lead-time clause (add <PricingNote />)"
            )
    return violations
...
def run_tests() -> int:
    ...
    for name, src, should_pass in cases:
        ...
        has_signal = bool(PRICE_RE.search(body) or LEADTIME_RE.search(body))
        if is_contract:
            ok = ("<PricingNote" not in src) or has_optout(src)
        else:
...
if __name__ == "__main__":
    ...
    if v:
        ...
        sys.exit(1)
    print(
        "✅ §287-T turnaround-clause gate: all price/turnaround pages carry the machine-dependent lead-time clause; contract pages opted out."
    )
    sys.exit(0)
