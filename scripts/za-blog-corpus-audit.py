...
def gsc():
    ...
    if GSC_CSV.exists():
        for r in csv.DictReader(open(GSC_CSV)):
            ...
            if m:
                g[m.group(1)] = (int(r["Clicks"]), int(r["Impressions"]))
    return g
...
def main():
    ...
    for slug, body in P.items():
        ...
        perm_shape = any(slug.startswith(m + "-") for m in MODELS) and not slug.endswith(
            "-johannesburg"
        )
        ...
        if invented > 0:
            bucket = "INVENTED-PRICE-LIVE (§489)"
        elif weak:
            bucket = "WEAK-PRICING (§172)"
        elif doorway:
            bucket = "DOORWAY (§529)"
        elif perm_shape:
            bucket = "PERMUTATION-SHAPE (§167)"
        else:
            bucket = "COMPLIANT"
        rows.append(
            dict(
    ...
    def section(title, pred, rank_desc=True):
        ...
        earn = [r for r in sel if r["traffic"] > 0]
        dead = [r for r in sel if r["traffic"] == 0]
        out.append(
            f"   earners (clicks/imp>0, KEEP+fix): {len(earn)}  |  zero-traffic (301/noindex candidate): {len(dead)}"
        )
        for r in sel[:25]:
            out.append(
                f"   - {r['clk']}clk {r['imp']}imp  `{r['slug']}`"
                + (f"  [{r['rt']}]" if r["rt"] != "unknown" else "")
            )
        out.append("")

    section(
        "INVENTED-PRICE-LIVE (§489) — real live exposure, scrub earners first",
        lambda r: r["invented"] > 0,
    )
    section(
        "WEAK-PRICING (§172) — anchor earners / 301 deadweight",
        lambda r: r["bucket"].startswith("WEAK"),
    )
    section(
        "DOORWAY (§529) — keep earners, 301 zero-traffic",
        lambda r: r["bucket"].startswith("DOORWAY"),
    )
    section(
        "PERMUTATION-SHAPE (§167) — GSC-ranked, never blanket-purge",
        lambda r: r["bucket"].startswith("PERMUTATION"),
    )
    ...
    out.append(
        "- demand-gated engine: no model×fault permutation enters the pool without GSC demand"
    )
    out.append(
        "- review-queue (default-safe): nothing auto-publishes; human approves first-hand value (§167)"
    )
    out.append(
        "- single canonical generator → single filtered inserter (6 bypass generators retired)"
    )
    rep = "\n".join(out)
    dest = WEB / "docs/seo/blog-corpus-compliance-audit-29-06-2026.md"
    dest.write_text(rep + "\n")
    print(rep)
    print(f"\n[written] {dest}")
