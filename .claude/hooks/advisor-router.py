...
"""
import argparse, collections, json, os, sys, time, urllib.request, urllib.error
...
ADVISORS = {
    "lint": {
        ...
        "system": ("You explain a single linter error. Answer in at most three lines: "
    ...
    "secrets": {
        ...
        "system": ("You judge whether a flagged string is a live credential or a test "
    ...
    "test-failure": {
        ...
        "system": ("You classify one test failure. First word must be CODE, TEST, or "
    ...
    "divergence": {
        ...
        "system": ("You summarise a diff between two copies of the same file. Three bullets: "
    ...
    "changelog": {
        ...
        "system": ("You classify one changelog entry for a repository that uses Claude Code "
    ...
    "commit": {
        ...
        "system": ("Write one conventional commit subject line, under 72 characters, no body, "
...
def log(m): print(f"[advisor] {m}", file=sys.stderr)
...
def classify(question):
    ...
    table = [
        ("divergence", ("diverg", "duplicate", "two copies", "diff between",
                        "which version", "which copy", "same file in")),
...
def ask(question, forced=None):
    ...
    if not name or name not in ADVISORS:
        return {"advisor": None, "routed_by": how, "answer": "NEEDS_ESCALATION",
                "reason": "no specialist matched; route to Claude Code"}
    a = ADVISORS[name]
    try:
        out = ollama(a["model"], a["system"] + " " + a["format"], question)
    except Exception as e:
        return {"advisor": name, "routed_by": how, "answer": "NEEDS_ESCALATION",
    ...
    return {"advisor": name, "routed_by": how, "model": a["model"],
            "answer": out, "escalated": escalate}


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--ask"); ap.add_argument("--advisor")
    ...
    if a.self_test:
        ...
        chk("positive/routes-secrets", classify("is this AKIA string a real credential")[0] == "secrets")
        chk("positive/routes-changelog", classify("this changelog entry deprecates a hook")[0] == "changelog")
        ...
        with tempfile.TemporaryDirectory() as t:
            ...
            chk("absence/derive-with-no-telemetry", d["total_records"] == 0 and len(d["ranked"]) == len(ADVISORS))
            # POSITIVE: derive ranks by real frequency
            tf = Path(t) / "tel.ndjson"
            tf.write_text("\n".join(json.dumps({"hook": h, "verdict": "fail"})
                                    for h in ["lint-v2"] * 5 + ["secrets"] * 2))
            d2 = derive(tf)
            chk("positive/derive-ranks-by-frequency", d2["ranked"][0] == "lint" and d2["counts"]["lint"] == 5)
        ...
        chk("safety/all-advisors-bounded",
            all(len(v["system"]) < 600 for v in ADVISORS.values()))
        chk("safety/all-advisors-can-escalate",
    ...
    if a.ask:
        ...
        print(json.dumps(r, indent=2) if a.json else
              f"advisor: {r.get('advisor')} (routed by {r.get('routed_by')})\n\n{r.get('answer')}"
