...
def get_ms_token():
    r = requests.post(
        ...
        data={
            ...
            "client_id": os.environ.get("MS_CLIENT_ID", "033c8fd5-10b5-4918-9030-5a336531066f"),
...
def run(chat_id=""):
    ...
    with open(
        os.path.join(REPORTS, f"report-{datetime.now().strftime('%Y%m%d-%H%M')}.md"), "w"
    ) as f:
        f.write(report)
    ...
    if token:
        requests.post(
            ...
            json={
                "message": {
                    "subject": f"Claude Code Report - {chat_id} - {ts}",
