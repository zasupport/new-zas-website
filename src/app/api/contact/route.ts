...
export async function POST(request: NextRequest) {
	try {
		...
		if (looksLikeBot(String(name), String(email), String(issue))) {
			console.log(
				`[contact] bot-filtered: name="${String(name).slice(0, 20)}" email="${String(email).slice(0, 30)}"`,
			);
		...
		if (clampLength(issueStr, LIMITS.issue) === null)
			return NextResponse.json(
				{ error: "Issue description too long (2000 chars max)" },
				{ status: 400 },
			);
		...
		const whatsappUrl = phoneStr
			? buildClientReplyUrl(nameStr, phoneStr, issueStr)
			: "";
