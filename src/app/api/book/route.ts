...
export async function POST(request: NextRequest) {
	...
	if (_hp) {
		return NextResponse.json({
			success: true,
			whatsappUrl: `https://wa.me/${ZA_PHONE}`,
		});
	}
	...
	if (clampLength(deviceTypeStr, LIMITS.deviceType) === null)
		return NextResponse.json(
			{ error: "Device type too long" },
			{ status: 400 },
		);
	if (clampLength(problemStr, LIMITS.problem) === null)
		return NextResponse.json(
			{ error: "Problem description too long (2000 chars max)" },
			{ status: 400 },
		);
	if (clampLength(preferredContactStr, LIMITS.preferredContact) === null)
		return NextResponse.json(
			{ error: "Preferred contact value too long" },
			{ status: 400 },
		);
