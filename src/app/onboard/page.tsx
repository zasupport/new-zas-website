...
const C = {
  ...
  warningBg: 'rgba(251,191,36,0.05)', warningBorder: 'rgba(251,191,36,0.12)', warningText: '#fbbf24',
};
...
type DoctorKey = 1 | 2 | 3;

function calc(base: number) {
  const vat = Math.round(base * 0.15);
  const subtotal = base + vat;
  const proc = Math.ceil(subtotal * 0.03 + 1.5);
  return { base, vat, subtotal, proc, total: subtotal + proc };
}
...
const SERVICES = [
  ...
  { name: 'Security and Backup Verification', detail: 'Monthly verification that all backup systems are functioning correctly, data integrity checks, and restore testing to ensure your practice data is recoverable.' },
  ...
  { name: 'One Physical Onsite Visit p/m', detail: 'One scheduled onsite visit per month to your practice. Hardware inspection, network checks, physical security review, and hands-on support.' },
...
const STEPS = ['Your Details', 'Review & Confirm', 'Payment'];
...
function Card({ children, onClick, selected, style }: { children: React.ReactNode; onClick?: () => void; selected?: boolean; style?: React.CSSProperties }) {
  return (
    <div onClick={onClick} style={{ background: selected ? C.glow : C.cardBg, border: `1px solid ${selected ? C.greenBorder : C.cardBorder}`, borderRadius: 16, padding: 20, cursor: onClick ? 'pointer' : 'default', transition: 'all 0.25s ease', ...style }}>
      {children}
    </div>
  );
}
...
function ServiceItem({ name, detail }: { name: string; detail: string }) {
  ...
  return (
    <div style={{ borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
      ...
      {open && <div style={{ padding: '0 0 12px 26px', fontSize: 12, color: C.muted, fontFamily: f, lineHeight: 1.6 }}>{detail}</div>}
    </div>
  );
}
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: 0, marginBottom: 32 }}>
      {STEPS.map((label, i) => (
        <div key={label} style={{ display: 'flex', alignItems: 'center', flex: i < 2 ? 1 : 'unset' }}>
          ...
          {i < 2 && <div style={{ flex: 1, height: 1, background: i < step ? C.green : 'rgba(255,255,255,0.06)', margin: '0 12px' }} />}
        </div>
      ))}
    </div>
  );
...
function Step1({ data, setData, onNext }: { data: FormData; setData: (d: FormData) => void; onNext: () => void }) {
  const [err, setErr] = useState<Record<string, string>>({});
  const validate = () => {
    const e: Record<string, string> = {};
    if (!data.practiceName?.trim()) e.practiceName = 'Required';
    if (!data.contactName?.trim()) e.contactName = 'Required';
    if (!data.email?.trim()) e.email = 'Required';
    else if (!/\S+@\S+\.\S+/.test(data.email)) e.email = 'Invalid email';
    if (!data.phone?.trim()) e.phone = 'Required';
    if (!data.doctorCount) e.dc = 'Please select';
    setErr(e);
  ...
  return (
    <div>
      ...
      <h2 style={{ fontSize: 22, fontWeight: 700, color: C.text, fontFamily: f, marginBottom: 4 }}>Your Details</h2>
      <p style={{ fontSize: 13, color: C.muted, fontFamily: f, marginBottom: 24 }}>Tell us about your practice. Takes about 2 minutes.</p>
      <Input label="Practice / Billing Entity" value={data.practiceName || ''} onChange={v => setData({ ...data, practiceName: v })} placeholder="e.g. Prodehl Medical Practice (Pty) Ltd" required error={err.practiceName} hint="The entity name that will appear on invoices" />
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        <Input label="Contact Person" value={data.contactName || ''} onChange={v => setData({ ...data, contactName: v })} placeholder="e.g. Dr Leanne Prodehl" required error={err.contactName} />
        <Input label="Phone" value={data.phone || ''} onChange={v => setData({ ...data, phone: v })} placeholder="e.g. 082 123 4567" type="tel" required error={err.phone} />
      </div>
      <Input label="Email" value={data.email || ''} onChange={v => setData({ ...data, email: v })} placeholder="e.g. info@practice.co.za" type="email" required error={err.email} />
      <div style={{ marginBottom: 20 }}>
        <label style={{ display: 'block', fontSize: 11, fontWeight: 600, color: C.muted, marginBottom: 8, fontFamily: f, letterSpacing: '0.06em', textTransform: 'uppercase' }}>
          Number of Doctors <span style={{ color: '#f87171' }}>*</span>
        </label>
        <div style={{ display: 'flex', gap: 10 }}>
          {([1, 2, 3] as DoctorKey[]).map(n => {
            const sel = data.doctorCount === n;
            const p = PRICING[n];
            const c = calc(p.base);
            return (
              <Card key={n} onClick={() => setData({ ...data, doctorCount: n })} selected={sel} style={{ flex: 1, textAlign: 'center', padding: 14 }}>
                <div style={{ fontSize: 24, fontWeight: 800, color: sel ? C.green : C.muted, fontFamily: f, lineHeight: 1 }}>{n}{n === 3 ? '+' : ''}</div>
                <div style={{ fontSize: 10, color: C.muted, fontFamily: f, marginTop: 2 }}>{p.sub}</div>
                <div style={{ fontSize: 13, fontWeight: 700, color: sel ? C.text : C.muted, fontFamily: f, marginTop: 6 }}>R {c.total.toLocaleString()}<span style={{ fontSize: 9, fontWeight: 400 }}>/mo incl.</span></div>
              </Card>
            );
          })}
        </div>
        {err.dc && <span style={{ fontSize: 11, color: '#f87171', display: 'block', marginTop: 6, fontFamily: f }}>{err.dc}</span>}
      </div>
      <Input label="Physical Address (optional)" value={data.address || ''} onChange={v => setData({ ...data, address: v })} placeholder="Street, suburb, city" />
...
function Step2({ data, onNext, onBack }: { data: FormData; onNext: () => void; onBack: () => void }) {
  const key = Math.min(Math.max(data.doctorCount ?? 1, 1), 3) as DoctorKey;
  const p = PRICING[key];
  const c = calc(p.base);
  return (
    <div>
      <h2 style={{ fontSize: 22, fontWeight: 700, color: C.text, fontFamily: f, marginBottom: 4 }}>Review & Confirm</h2>
      <p style={{ fontSize: 13, color: C.muted, fontFamily: f, marginBottom: 24 }}>Confirm your details and package before payment.</p>
      <Card style={{ marginBottom: 14 }}>
        <div style={{ fontSize: 10, fontWeight: 600, color: C.muted, fontFamily: f, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 10 }}>Practice</div>
        {([['Billing Entity', data.practiceName], ['Contact', data.contactName], ['Email', data.email], ['Phone', data.phone], ['Doctors', data.doctorCount === 3 ? '3+' : String(data.doctorCount)]] as [string, string][]).map(([k, v]) => (
          <div key={k} style={{ display: 'flex', justifyContent: 'space-between', padding: '5px 0' }}>
            <span style={{ fontSize: 13, color: C.muted, fontFamily: f }}>{k}</span>
            <span style={{ fontSize: 13, color: C.text, fontFamily: f, fontWeight: 500 }}>{v}</span>
          </div>
        ))}
      </Card>
      <Card style={{ marginBottom: 14, position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 4 }}>
          <div>
            <div style={{ fontSize: 17, fontWeight: 700, color: C.text, fontFamily: f }}>{p.label} SLA</div>
            <div style={{ fontSize: 12, color: C.muted, fontFamily: f, marginTop: 2 }}>{p.sub} · Medical Practice Protection</div>
          </div>
          <div style={{ textAlign: 'right' }}>
            <div style={{ fontSize: 22, fontWeight: 800, color: C.green, fontFamily: f }}>R {c.total.toLocaleString()}</div>
            <div style={{ fontSize: 10, color: C.muted, fontFamily: f }}>/month incl. VAT + fees</div>
          </div>
        </div>
      </Card>
      <Card style={{ marginBottom: 14, padding: '8px 20px' }}>
      ...
      <Card style={{ marginBottom: 14 }}>
        ...
        <div style={{ display: 'grid', gap: 4 }}>
          {([['Monthly SLA fee', `R ${c.base.toLocaleString()}`], ['VAT (15%)', `R ${c.vat.toLocaleString()}`], ['Payment processing (3% + R1.50)', `R ${c.proc}`]] as [string, string][]).map(([k, v]) => (
            <div key={k} style={{ display: 'flex', justifyContent: 'space-between', padding: '3px 0' }}>
              <span style={{ fontSize: 12, color: C.muted, fontFamily: f }}>{k}</span>
          ...
          <div style={{ borderTop: `1px solid ${C.cardBorder}`, marginTop: 6, paddingTop: 8, display: 'flex', justifyContent: 'space-between' }}>
            ...
            <span style={{ fontSize: 18, fontWeight: 800, color: C.green, fontFamily: f }}>R {c.total.toLocaleString()}</span>
          </div>
        </div>
      </Card>
      <div style={{ display: 'flex', gap: 10, padding: '10px 14px', background: C.glow, border: `1px solid ${C.greenBorder}`, borderRadius: 12, marginBottom: 10 }}>
        <span style={{ fontSize: 15, flexShrink: 0 }}>📅</span>
        <div style={{ fontSize: 12, color: C.text, fontFamily: f, lineHeight: 1.5 }}><strong>Billing date: 25th monthly.</strong> First charge today, recurring on the 25th.</div>
      </div>
      <div style={{ display: 'flex', gap: 10, padding: '10px 14px', background: C.warningBg, border: `1px solid ${C.warningBorder}`, borderRadius: 12, marginBottom: 12 }}>
        <span style={{ fontSize: 15, flexShrink: 0 }}>⚠️</span>
        <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.5)', fontFamily: f, lineHeight: 1.5 }}>
          Medical practices face penalties of up to <strong style={{ color: C.warningText }}>R 10 million</strong> for POPIA non-compliance.
        </div>
      </div>
      <div style={{ fontSize: 11, color: C.muted, fontFamily: f, lineHeight: 1.6, marginBottom: 20 }}>
        12-month service agreement. 30-day cancellation notice. Service commences upon Digital Risk Assessment. Contract emailed to {data.email} upon payment.
      </div>
      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
        <Btn variant="secondary" onClick={onBack}>Back</Btn>
        <Btn onClick={onNext}>Proceed to Payment</Btn>
      </div>
    </div>
  );
}

function Step3({ data, onBack, checkoutId, widgetUrl }: { data: FormData; onBack: () => void; checkoutId: string; widgetUrl: string }) {
  const [agreed, setAgreed] = useState(false);
  const [scriptReady, setScriptReady] = useState(false);
  const key = Math.min(Math.max(data.doctorCount ?? 1, 1), 3) as DoctorKey;
  const c = calc(PRICING[key].base);
  const siteUrl = process.env.NEXT_PUBLIC_SITE_URL ?? 'https://zasupport.com';

  useEffect(() => {
    if (scriptReady && checkoutId) {
      const el = document.getElementById('peach-widget-script-gen');
      if (el) el.remove();
      const script = document.createElement('script');
      script.id = 'peach-widget-script-gen';
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 13, color: C.muted, fontFamily: f, marginBottom: 20 }}>Secure payment via Peach Payments. Card tokenised for recurring billing on the 25th.</p>
      <Card style={{ marginBottom: 14, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div><span style={{ fontSize: 13, fontWeight: 600, color: C.text, fontFamily: f, display: 'block' }}>First payment today</span><span style={{ fontSize: 10, color: C.muted, fontFamily: f }}>Then R {c.total.toLocaleString()} on the 25th</span></div>
        <div style={{ fontSize: 24, fontWeight: 800, color: C.green, fontFamily: f }}>R {c.total.toLocaleString()}</div>
      </Card>
      <div style={{ fontSize: 11, color: C.muted, fontFamily: f, marginBottom: 14, padding: '0 2px' }}>
        Billing to: <strong style={{ color: C.text }}>{data.practiceName}</strong> · {data.email}
      </div>
      <div style={{ display: 'flex', gap: 10, alignItems: 'flex-start', marginBottom: 20, cursor: 'pointer' }} onClick={() => setAgreed(!agreed)}>
        ...
        <span style={{ fontSize: 11, color: C.muted, fontFamily: f, lineHeight: 1.5 }}>
          I agree to the ZA Support SLA. I authorise R {c.total.toLocaleString()} today and R {c.total.toLocaleString()} on the 25th monthly. 12-month agreement, 30-day cancellation notice. Includes processing fees (3% + R1.50).
        </span>
      </div>
      {agreed && checkoutId ? (
        <Card style={{ marginBottom: 14 }}>
          ...
          <form action={`${siteUrl}/onboard/done`} className="paymentWidgets" data-brands="VISA MASTER AMEX" data-checkout-id={checkoutId} style={{ color: C.text }} />
        </Card>
      ) : (
...
export default function SLAOnboarding() {
  const [step, setStep] = useState(0);
  const [data, setData] = useState<FormData>({ practiceName: '', contactName: '', email: '', phone: '', doctorCount: null, address: '' });
  ...
  const handleStep2to3 = useCallback(async () => {
    setLoading(true);
    setErr('');
    try {
      const onboardRes = await fetch('/api/sla/onboard', {
        ...
        body: JSON.stringify({ practice_name: data.practiceName, contact_name: data.contactName, email: data.email, phone: data.phone, doctor_count: data.doctorCount, address: data.address }),
      });
      if (!onboardRes.ok) {
    ...
    <div style={{ minHeight: '100vh', background: C.bgDark }}>
      ...
      <div style={{ position: 'relative', zIndex: 1, maxWidth: 540, margin: '0 auto', padding: '28px 20px 48px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 28 }}>
          <Logo />
          <span style={{ fontSize: 11, color: C.muted, fontFamily: f }}>SLA Onboarding</span>
        </div>
        <Progress step={step} />
        {err && (
          <div style={{ marginBottom: 16, padding: '10px 14px', background: 'rgba(248,113,113,0.08)', border: '1px solid rgba(248,113,113,0.2)', borderRadius: 10, fontSize: 12, color: '#f87171', fontFamily: f }}>{err}</div>
        )}
        <div style={{ opacity: vis ? 1 : 0, transform: vis ? 'translateY(0)' : 'translateY(8px)', transition: 'all 0.35s ease' }}>
          ...
          {step === 1 && <Step2 data={data} onNext={handleStep2to3} onBack={() => setStep(0)} />}
          {step === 2 && <Step3 data={data} onBack={() => setStep(1)} checkoutId={checkoutId} widgetUrl={widgetUrl} />}
        </div>
        {loading && (
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(10,26,24,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
            <div style={{ textAlign: 'center' }}>
              <div style={{ width: 36, height: 36, border: `3px solid rgba(15,234,122,0.1)`, borderTopColor: C.green, borderRadius: '50%', margin: '0 auto 12px', animation: 'spin 0.7s linear infinite' }} />
              <style>{`@keyframes spin{to{transform:rotate(360deg)}}`}</style>
              <div style={{ fontSize: 14, color: C.text, fontFamily: f }}>Preparing payment…</div>
            </div>
          </div>
        )}
