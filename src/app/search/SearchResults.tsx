...
function getIntentCTA(query: string): IntentCTA | null {
	...
	const isRepair =
		/logic board|liquid|water damage|screen|display|battery|macbook repair|mac repair|not turning on|won.t start/.test(
			q,
		);
	const isManaged =
		/managed|sla|monthly|business it|it support|it services/.test(q);
	const isHealthCheck =
		/health check|diagnostic|slow mac|performance|virus|malware|cybershield|security/.test(
			q,
		);
	...
	if (isManaged)
		return {
			...
			secondary: {
				...
				href: buildWhatsAppUrl("SEARCH", "general"),
			},
		};
	if (isHealthCheck)
		return {
...
// Build-time static index (public/search-index.json), generated by
// seo/scripts/build-search-index.mjs. Keyless + deterministic, replaces the
// external Google-CSE dependency that returned HTTP 500 in production.
...
function rankIndex(entries: IndexEntry[], query: string): SearchResult[] {
	const nq = norm(query);
	if (!nq) return [];
	...
	const scored = entries
		.map((e) => {
			...
			// slight preference for hubs/services over deep permutation pages
			if (
				score > 0 &&
	...
	return scored.map(({ e }) => ({
		title: e.title,
		url: e.url,
		snippet: `${e.title}, ZA Support, Johannesburg. ${e.keywords.replace(/\b\w/g, (c) => c.toUpperCase())}.`,
	}));
}
...
function CTACard({ cta }: { cta: IntentCTA }) {
	...
	return (
		<div className="bg-[rgba(15,234,122,0.06)] border border-[rgba(15,234,122,0.2)] rounded-xl p-5">
			<p className="text-[10px] font-semibold uppercase tracking-wider text-[#0FEA7A]/60 mb-2">
				Recommended for you
			</p>
...
export default function SearchResults({ query, start }: Props) {
	...
	useEffect(() => {
		...
		fetch("/search-index.json")
			.then((r) => r.json())
			.then((idx: { entries: IndexEntry[] }) => {
				if (cancelled) return;
				const ranked = rankIndex(idx.entries || [], query);
				setData({ total: ranked.length, results: ranked });
				setLoading(false);
			})
			.catch(() => {
				if (cancelled) return;
			...
			<div className="space-y-6">
				<div className="text-sm text-white/50 space-y-2">
					...
					<p>
						...
						<Link href="/contact" className="text-[#0FEA7A] hover:underline">
							contact us directly
						</Link>
						.
					</p>
	...
	const pageResults = data.results.slice(start - 1, start - 1 + 10);
			...
			<div className="space-y-8">
				{pageResults.map((r) => {
					...
					const path = isInternal
						? r.url.replace(/^https?:\/\/zasupport\.com/, "") || "/"
						: null;

					return (
						<div key={r.url} className="group">
							<div className="text-xs text-[#0FEA7A]/70 mb-0.5 truncate">
								{displayUrl}
							</div>
							{path ? (
